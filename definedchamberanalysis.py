...
class RocketEngine:
    def __init__(self, oxName, fuelName, Pc, Pe, MR, radius_cylinder, length_chamber, radius_throat, radius_exit, cdo, cdf, Pi, density_o, density_f):
        self.oxName = oxName
        ...
        self.radius_throat = radius_throat
    ...
    def runSim(self):
        self.At = self.radius_throat**2 * np.pi
        ...
        self.mdot = self.At * self.Pt * 1e5 / np.sqrt(self.Tt) * np.sqrt(self.gt / self.Rt)
        self.thrust = self.mdot * self.ve
...
thanos = RocketEngine(
    ...
    length_chamber=0.15,
    radius_throat=21.243/1000,
    radius_exit=41.633/1000,
    cdo = 0.4,
    cdf = 0.7,
    Pi = 30,
    density_o = 800,
...
n_it = 10
waterlist = np.linspace(0, 25, n_it)
oxflux = np.zeros(n_it)
fuelflux = np.zeros(n_it)
fduration = np.zeros(n_it)
oxduration = np.zeros(n_it)
thrust = np.zeros(n_it)
for i, waterperc in enumerate(waterlist):
    ...
    thrust[i] = thanos.thrust
    
    coolantdensity = waterperc / 100 * 1000 + (100 - waterperc) / 100 * 800
    oxdensity = CP.PropsSI('D','T',273 + 10,'Q',0,'NitrousOxide')
    oxduration[i] = 0.012 * oxdensity / oxflux[i]
    fduration[i] = 0.008 * coolantdensity / fuelflux[i]
ax[0].plot(waterlist, oxflux,label="Ox")
ax[0].plot(waterlist, fuelflux,label="Fuel")
ax[1].plot(waterlist, oxduration,label="Ox")
ax[1].plot(waterlist, fduration,label="Fuel")
ax[2].plot(waterlist, thrust)
ax[0].legend()
ax[1].legend()
ax[0].grid()
ax[1].grid()
